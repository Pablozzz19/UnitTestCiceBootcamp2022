...
class UnitTestPresenterTests: XCTestCase {
    ...
    func test_Sumar_InputCorrecto_OK_Result() {
        ...
        let result: Int
        // 2 -> Action / WHEN
        result = presenter.sumar(numeor1: 7, numero2: 4)
        // 3 -> Assert / THEN
    ...
    func test_Restar_InputCorrecto_OK_Result() {
        ...
        let result: Int
        result = presenter.restar(numeor1: 7, numero2: 4)
        XCTAssertEqual(resultExpected, result)
    }
    ...
    func test_Restar_InputCorrecto_Two_OK_Result() {
        ...
        let result: Int
        result = presenter.restar(numeor1: 10, numero2: 20)
        XCTAssertEqual(resultExpected, result)
    }
    
    func test_Dividir_InputCorrecto_OK_Result() {
        XCTAssertEqual(2, presenter.dividir(numeor1: 4, numero2: 2))
    }
    
    func test_Dividir_InputIncorrect_OK_Result() {
        XCTAssertEqual(2, try presenter.dividirCero(numeor1: 4, numero2: 2))
        XCTAssertThrowsError(try presenter.dividirCero(numeor1: 4, numero2: 0))
    }
    ...
    func test_Assert_Types() {
        ...
        XCTAssertEqual(self.presenter.sumar(numeor1: 2, numero2: 2), 4)
        XCTAssertEqual("Pablo", "Pablo")
        XCTAssertGreaterThan(10, 1)
    }
    
    func test_Performance_Sut_Dividir() {
        self.measure {
            self.presenter.dividir(numeor1: 4, numero2: 2)
        }
    }
